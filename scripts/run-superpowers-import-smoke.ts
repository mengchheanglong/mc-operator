...
type SmokeReport = {
  suite: "superpowers-import-smoke";
  ok: boolean;
  generatedAt: string;
...
function reportPaths() {
  ...
  return {
    ...
    latestPath: path.join(reportsDir, "superpowers-latest.json"),
    archivePath: path.join(reportsDir, `superpowers-${isoStamp(generatedAt)}.json`),
  };
}
...
async function run() {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), "mission-control-superpowers-import-"));
  const sqlitePath = path.join(tempDir, "superpowers-import-smoke.sqlite");
  const workspaceRoot = path.resolve(process.cwd(), "..");
  const backendPort = 3781 + Math.floor(Math.random() * 200);
  ...
  try {
    ...
    const defaultImportJson = (await defaultImportResponse.json()) as { agents?: ImportedAgent[] };
    const defaultImportSuperpowersCount = Array.isArray(defaultImportJson.agents)
      ? defaultImportJson.agents.filter((agent) => agent.sourcePack === "superpowers").length
      : 0;
    assert.equal(
      defaultImportSuperpowersCount,
      0,
      "expected superpowers to remain excluded from default import",
    );
    ...
    assert.equal(importResponse.status, 200, "expected explicit superpowers import to return 200");
    const importJson = (await importResponse.json()) as {
      agents?: ImportedAgent[];
      updatedCount?: number;
    };
    const importedAgent = Array.isArray(importJson.agents)
      ? importJson.agents.find((agent) => agent.sourcePack === "superpowers")
      : undefined;
    assert.ok(importedAgent, "expected imported superpowers agent");
    assert.equal(importedAgent?.name, "Superpowers Workflow Operator");
    assert.equal(importedAgent?.sourceRef, "superpowers/workflow-operator");
    assert.equal(importedAgent?.workflowProfile?.mode, "execution");
    ...
    const assetLabels = Array.isArray(importedAgent?.packAssets)
      ...
      : [];
    for (const requiredLabel of ["README.md", "skills", "commands", "docs", "agents"]) {
    ...
    assert.ok((syncJson.updatedCount || 0) >= 1, "expected syncExisting superpowers import to update existing agent");
