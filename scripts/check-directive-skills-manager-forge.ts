...
function main() {
  ...
  const profile = getForgePromotionProfile("skill_lifecycle_guard/v1");

  const forgeRecordPath = path.join(
    directiveRoot,
    "forge",
    "records",
    "2026-03-21-skills-manager-forge-record.md",
  );
  const proofPath = path.join(
    directiveRoot,
    "forge",
    "records",
    "2026-03-21-skills-manager-runtime-slice-01-proof.md",
  );
  const promotionPath = path.join(
    directiveRoot,
    "forge",
    "promotion-records",
    "2026-03-21-skills-manager-promotion-record.md",
  );
  const registryPath = path.join(
    directiveRoot,
    "forge",
    "registry",
    "2026-03-21-skills-manager-registry-entry.md",
  );
  ...
  const latestReportPath = path.join(
    ...
    "reports",
    "agent-pack-imports",
    "skills-manager-latest.json",
  );
  const packRoot = getForgeSourcePackPath("skills-manager");
  const packReadyPath = path.join(packRoot, "SOURCE_PACK_READY.md");
  const sourcePackEntry = getForgeSourcePackCatalogEntry("skills-manager");
  ...
  if (contract) {
    const required = includesAll(contract, [
      ...
      "`skills-manager/lifecycle-operator`",
    ]);
    checks.push({
      id: "skills-manager-contract-required-terms",
      ok: required.ok,
  ...
  if (forgeRecord) {
    const required = includesAll(forgeRecord, [
      "bounded skill lifecycle import lane",
    ...
    checks.push({
      id: "skills-manager-forge-record-required-terms",
      ok: required.ok,
  ...
  if (proof) {
    ...
    checks.push({
      id: "skills-manager-proof-required-terms",
      ok: required.ok,
  ...
  if (promotion) {
    const required = includesAll(promotion, [
      ...
      profile.proofShape,
      profile.primaryHostCheckCommand,
      "callable (bounded-skill-lifecycle-lane)",
      "check:agents-import-packs-api-backend",
      "check:directive-skills-manager-forge",
    ]);
    checks.push({
      id: "skills-manager-promotion-required-terms",
      ok: required.ok,
  ...
  if (registry) {
    ...
    checks.push({
      id: "skills-manager-registry-required-terms",
      ok: required.ok,
  ...
  if (importPacksService) {
    const required = includesAll(importPacksService, [
      "\"skills-manager\"",
      "Skills Lifecycle Operator",
      "skills-manager/lifecycle-operator",
      "resolveSkillsManagerRootFromBackendCwd",
    ]);
    checks.push({
      id: "skills-manager-import-service-required-terms",
      ok: required.ok,
  ...
  if (latestReport) {
    const parsed = JSON.parse(latestReport) as SkillsManagerSmokeReport;
    ...
    checks.push({
      id: "skills-manager-latest-report-thresholds",
      ok:
        parsed.suite === "skills-manager-import-smoke" &&
        parsed.ok === true &&
        parsed.sourcePack === "skills-manager" &&
        Number(parsed.importedCount ?? 0) >= 1 &&
        Number(parsed.updatedCount ?? 0) >= 1 &&
        parsed.importedAgent?.name === "Skills Lifecycle Operator" &&
        parsed.importedAgent?.sourcePack === "skills-manager" &&
        parsed.importedAgent?.sourceRef === "skills-manager/lifecycle-operator" &&
        parsed.importedAgent?.workflowMode === "execution" &&
        ["README.md", "scripts", "src", "assets"].every((label) => assetLabels.includes(label)) &&
        archivePathOk,
      reason:
        parsed.suite === "skills-manager-import-smoke" &&
        parsed.ok === true &&
        ...
        archivePathOk
          ? null
          : `latest report invalid: ok=${parsed.ok} importedCount=${parsed.importedCount} updatedCount=${parsed.updatedCount} sourcePack=${parsed.sourcePack} agent=${JSON.stringify(parsed.importedAgent)} archivePathOk=${archivePathOk} error=${parsed.error || "none"}`,
    });
  }
