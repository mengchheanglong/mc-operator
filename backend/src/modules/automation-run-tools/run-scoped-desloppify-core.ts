...
export function evaluateLengthGate(input: {
  minChars?: number;
  content?: string;
}): LengthGateResult {
  const minCharsRaw = Number(input.minChars ?? 0);
  const minChars = Number.isFinite(minCharsRaw)
    ? Math.max(0, Math.floor(minCharsRaw))
    : 0;
...
export function classifyDesloppifyFailure(input: {
  timedOut?: boolean;
  exitCode?: number;
  stderr?: string;
}): DesloppifyFailureClass {
  if (input.timedOut) return "timeout";
  const stderr = String(input.stderr || "").toLowerCase();
  if (
    stderr.includes("no module named desloppify") ||
...
export function normalizeDesloppifyFailureClass(
  error: unknown,
): DesloppifyFailureClass {
  ...
  if (
    lower.includes("no module named desloppify") ||
...
async function runWithPythonBinary(input: {
  ...
  const env = {
    ...
    PYTHONPATH: process.env.PYTHONPATH
      ? `${path.resolve(input.toolRootPath)}${delimiter}${String(
