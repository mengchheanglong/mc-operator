...
const PROFILE_DIRECTORY_MAP: Record<AgencyAgentsProfile, string[]> = {
  ...
  product: ["product", "project-management", "strategy", "design", "support", "marketing", "sales"],
};
...
function sortSnapshotsByCreatedAt(
  snapshots: AgencyAgentsSnapshotSummary[],
  direction: "asc" | "desc",
) {
  ...
  return [...snapshots].sort((left, right) => {
    ...
    return left.snapshotId.localeCompare(right.snapshotId) * multiplier;
  });
}
...
function shouldExcludeRelative(relativePath: string) {
  ...
  return relativePath.split(path.sep).some((part) => shouldExcludePath(part) || part === MANIFEST_FILE);
}
...
async function collectManifestFiles(root: string, baseRoot = root): Promise<AgencyAgentsManifestFile[]> {
  if (!existsSync(root)) return [];
...
export async function buildAgencyAgentsManifest(input: {
  ...
  const selectedDirectories = [...(input.selectedDirectories || [])].sort((a, b) => a.localeCompare(b));
  const files = await collectManifestFiles(input.rootPath);
  const markdownFiles = files.filter((item) => item.path.toLowerCase().endsWith(".md")).length;
...
async function readManifest(rootPath: string): Promise<AgencyAgentsManifest | null> {
  ...
  try {
    ...
    if (parsed && parsed.version === 1 && Array.isArray(parsed.files) && typeof parsed.hash === "string") {
...
async function writeSnapshotIndex(snapshotRoot: string, index: AgencyAgentsSnapshotIndex) {
  await mkdir(snapshotRoot, { recursive: true });
  await writeFile(path.resolve(snapshotRoot, SNAPSHOT_INDEX_FILE), `${JSON.stringify(index, null, 2)}\n`, "utf8");
}

async function writeSnapshotPointer(snapshotRoot: string, snapshot: AgencyAgentsSnapshotSummary) {
  await mkdir(snapshotRoot, { recursive: true });
  await writeFile(path.resolve(snapshotRoot, SNAPSHOT_POINTER_FILE), `${JSON.stringify(snapshot, null, 2)}\n`, "utf8");
}

async function readSnapshotPointer(snapshotRoot: string): Promise<AgencyAgentsSnapshotSummary | null> {
...
async function pruneSnapshotHistory(
  snapshotRoot: string,
  index: AgencyAgentsSnapshotIndex,
): Promise<AgencyAgentsSnapshotIndex> {
  ...
  const newestFirst = sortSnapshotsByCreatedAt(index.snapshots, "desc");
  const keepSnapshotIds = new Set<string>();

  for (let idx = 0; idx < newestFirst.length; idx += 1) {
    const snapshot = newestFirst[idx];
    if (!snapshot) continue;
    if (idx < keepCount || idx < MIN_SNAPSHOT_KEEP_COUNT) {
      keepSnapshotIds.add(snapshot.snapshotId);
    }
  }

  const pruned = newestFirst.filter((snapshot) => !keepSnapshotIds.has(snapshot.snapshotId));
  for (const snapshot of pruned) {
    if (!isPathInside(snapshotRoot, snapshot.snapshotPath)) {
      continue;
    }
  ...
  return {
    version: 1,
    snapshots: index.snapshots.filter((snapshot) => keepSnapshotIds.has(snapshot.snapshotId)),
  };
}
...
async function createSnapshot(input: {
  ...
  await cp(input.sourceRoot, snapshotPath, {
    recursive: true,
    force: true,
    filter: (source) => !shouldExcludeRelative(path.relative(input.sourceRoot, source)),
  });
  ...
  const prunedSnapshotIndex = await pruneSnapshotHistory(input.snapshotRoot, snapshotIndex);
  await writeSnapshotIndex(input.snapshotRoot, prunedSnapshotIndex);
  const pointerTarget =
    sortSnapshotsByCreatedAt(prunedSnapshotIndex.snapshots, "desc")[0] || summary;
  await writeSnapshotPointer(input.snapshotRoot, pointerTarget);
  return summary;
}
...
export async function resolveAgencyAgentsSelection(input: {
  ...
  const selectedDirectories = explicit.length > 0
    ? [...explicitSet]
    : profile === "all"
  ...
  if (missing.length > 0) {
    throw new Error(`invalid_input: includeDirectories missing in source root -> ${missing.join(",")}`);
  }
...
export async function runAgencyAgentsSync(input: {
  ...
  if (!existsSync(input.sourceRoot)) {
    throw new Error(`source_missing: agency-agents source not found at ${input.sourceRoot}`);
  }
  ...
  const summary = await withTimeout({
    ...
    operation: (async () => {
      ...
      const chosen = requestedSnapshotId
        ? index.snapshots.find((entry) => entry.snapshotId === requestedSnapshotId) || null
        : resolveDefaultRollbackSnapshot(index) || pointer;

      if (!chosen) {
        throw new Error("snapshot_missing: no snapshots available for agency-agents rollback");
      }

      if (!existsSync(chosen.snapshotPath)) {
        throw new Error(`snapshot_missing: snapshot path not found -> ${chosen.snapshotPath}`);
      }
      ...
      if (!input.dryRun) {
        ...
        await cp(chosen.snapshotPath, input.targetRoot, {
          recursive: true,
          force: true,
          filter: (source) => !shouldExcludeRelative(path.relative(chosen.snapshotPath, source)),
        });
      ...
      return {
        ...
        manifestPath: input.dryRun ? chosen.manifestPath : path.resolve(input.targetRoot, MANIFEST_FILE),
