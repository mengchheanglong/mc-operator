...
export function resolveRunScopedToolId(value: unknown): RunScopedToolId {
  ...
  throw new Error(`invalid_input: unsupported toolId=${toolId || "(empty)"}; allowed=${allowlist.join(",")}`);
}

export function resolveCanonicalRunScopedToolId(toolId: RunScopedToolId): RunScopedToolId {
...
export function normalizeRunScopedToolInvocation(input: {
  ...
  const precheck = input.result.precheck || (metadata.precheck as RunScopedToolInvocationResult["precheck"] | undefined);
  ...
  return {
    ...
    runContext: {
      ...
      worktreePath: runContext.worktreePath ? String(runContext.worktreePath) : null,
    },
    precheck: precheck
      ? {
