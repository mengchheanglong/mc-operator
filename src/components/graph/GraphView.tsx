...
import { ExternalLink, Network, Pin, MousePointer2 } from "lucide-react";
  ...
  if (link.external) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex items-center gap-1.5 rounded-full border border-white/10 px-3 py-1.5 text-[11px] font-semibold text-slate-200 transition hover:border-blue-300/30 hover:text-white"
      >
        {link.label}
    ...
    <div ref={graphContainerRef} className="h-full w-full">
      ...
      >
        ...
        <Panel position="top-left">
          <div className="rounded-lg border border-white/10 bg-black/40 px-4 py-3 shadow-lg backdrop-blur">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.055] text-blue-100">
                <Network className="h-5 w-5" />
              </div>
              <div>
                <p className="text-sm font-semibold text-white">Project Map</p>
        ...
        <Panel position="top-right">
          <div className="w-[320px] rounded-lg border border-white/10 bg-black/40 px-4 py-3 shadow-lg backdrop-blur">
            <div className="flex items-center justify-between gap-3">
              ...
              {effectivePinnedNodeId ? (
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-white/10 px-2.5 py-1 text-[11px] font-semibold text-slate-300 transition hover:text-white"
                >
                  <Pin className="h-3 w-3" />
                  Clear pin
                </button>
              ) : (
                <div className="inline-flex items-center gap-1 rounded-full border border-white/10 px-2.5 py-1 text-[11px] font-semibold text-slate-300">
            ...
            {activeEntity?.kind === "document" ? (
              <p className="mt-3 text-xs leading-relaxed text-slate-500">
                Document role is inferred from wiki links, topic matches, and graph connectivity.
              </p>
            ) : null}
